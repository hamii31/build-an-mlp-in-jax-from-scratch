...
def init_linear_layer(key, in_dim, out_dim, scale=0.1):
    ...
    return {
        'W':jnp.multiply(sample_normal_matrix(key, (in_dim, out_dim)),scale),
...
def init_mlp_params(key, layer_sizes, scale=0.1):
    ...
    layers = []
    # don't forget to split the prng key into prng keys for each layer 
    key = split_prng_key(key, len(layer_sizes))
    for l in range(len(layer_sizes) - 1):
        layers.append(init_linear_layer(key[l], in_dim=layer_sizes[l], out_dim=layer_sizes[l+1], scale=scale))
    return layers

# Step 9 - linear_forward
def linear_forward(x, layer_params):
    # TODO: compute x @ W + b using layer_params['W'] and layer_params['b'].
    return jnp.array(x @ layer_params['W'] + layer_params['b'])
